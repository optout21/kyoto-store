...
use crate::{Config, FilterType, FilterStoreTrait, NoStorageFilterStore};
...
/// ```
#[derive(Debug)]
pub struct BuilderWithStore<S> where S: FilterStoreTrait {
    config: Config,
    network: Network,
...
impl<S> BuilderWithStore<S> where S: FilterStoreTrait {
    ...
    /// Consume the node builder and receive a [`Node`] and [`Client`].
    pub fn build_with_store<SS>(mut self) -> (Node<SS>, Client) where SS: FilterStoreTrait {
